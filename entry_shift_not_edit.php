...
<?php
		...
		$title = $_GET["title"];
		printf("<a href=\"./entry_group_list.php\">グループ選択画面に戻る</a><br />");
					...
					if(mysql_num_rows($rst_num)){
						...
						$rst = mysql_query($sql, $con);
				  		while($col = mysql_fetch_array($rst)){
							...
							output_for_leader($user_name, $decide_entry_type,$memo);
						}
						...
						if($NULL_staff_cnt !=0 && $AM_staff_cnt == 0 && $PM_staff_cnt == 0){
							printf("<td colspan=\"8\">0人</td><td></td></tr>");
						}elseif($FULL_staff_cnt != 0 && $AM_staff_cnt+$PM_staff_cnt == $FULL_staff_cnt*2){
							printf("<td colspan=\"8\">$PM_staff_cnt 人</td><td></td></tr>");
						}else{
							printf("<td colspan=\"3\">$AM_staff_cnt 人</td><td colspan=\"4\">$PM_staff_cnt 人</td><td></td></tr>");
						}
		...
		printf("<form method=\"get\" action=\"./testpdf.php\" name=\"form\">
