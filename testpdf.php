<?php
	...
	$sql = "select DISTINCT(name) from user inner join irregular_shift on user.id = user_id where day_id in 
	...
	fputs($fp, "<html><body>");
	fputs($fp, "<table><tr><td height=\"25\"><font size=\"20\">$title(仮)</font></td></tr><tr><td width=\"550\">");
	for($i=1; $i<=5; $i++){
		if($day_id[$i] != null){
			...
			$rst = mysql_query($sql, $con);
				...
				if(mysql_num_rows($rst_num)){
					fputs($fp, "<table border=\"1\"><tr align=\"center\"><td width=\"60\">$date</td><td width=\"60\">勤務タイプ</td>
								...
								<td width=\"40\">16:00</td><td width=\"100\">メモ</td></tr>");
					...
					$rst = mysql_query($sql, $con);
			  		while($col = mysql_fetch_array($rst)){
						...
						if($decide_entry_type == 1){
							...
							fputs($fp, "<tr align=\"center\"><td>$user_name</td><td>勤務希望なし</td>");
							for($time_cnt = 1; $time_cnt <=7; $time_cnt++){
								fputs($fp, "<td></td>");
							}
							fputs($fp, "<td>$memo</td></tr>");
	...
	fputs($fp, "<tr><td width=\"50\">スタッフ名</td><td width=\"40\">勤務日数</td><td width=\"40\">勤務時間</td></tr>");
	$cnt = 1;
	while($name_data[$cnt] != null){
		fputs($fp, "<tr><td width=\"50\">$name_data[$cnt]</td>
						<td width=\"40\">$week_work_cnt[$cnt]</td>
						<td width=\"40\">$week_work_time[$cnt]</td></tr>");
		$cnt++;
	}
	fputs($fp, "</table>");
